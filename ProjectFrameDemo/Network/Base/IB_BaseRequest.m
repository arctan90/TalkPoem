...
-(void)handleResponse:(id)responseObject isReadCache:(BOOL)isReadCache
{
    if ([responseObject isKindOfClass:[NSDictionary class]]) {
        ...
        if (baseModel.statusCode == kErrorCode_Success) {
            [self processResult:baseModel];
        }
        baseModel.isCache = isReadCache;
        IB_Error* bError = nil;
        if (_requestSuccFinishBlock && baseModel.statusCode == kErrorCode_Success) {
            _requestSuccFinishBlock(baseModel);
        }else if(_requestFailFinishBlock && baseModel.statusCode != kErrorCode_Success){
            bError = [[IB_Error alloc]initWithDomain:self.requestUrl code:baseModel.statusCode userInfo:@{@"description":@"ok "}];
            _requestFailFinishBlock(bError);
        }
        if (_finalBlock) {
